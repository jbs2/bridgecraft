...
int main( int argc, char *argv[] ) {
	...
	window.keys	= &keys;
	window.init.title	= (char*) "Bridge Craft";
	window.init.width = wwidth = 1680;//640;
	window.init.height = wheight = 1050;//480;
			...
			switch( event.type ) {
				...
				case ConfigureNotify:
					if( (event.xconfigure.width != window.init.width) ||
						...
						window.init.width = wwidth = event.xconfigure.width;
						window.init.height = wheight = event.xconfigure.height;
						ReshapeGL( window.init.width, window.init.height );
					}
					break;
				case ButtonPress:
